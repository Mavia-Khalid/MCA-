...
int check_pnr(int pnr);                     						 // Checks if PNR Exists or Not
int is_available(int train_num, vector<Trains> &train);              // Object as an Parameter Checks if Train No is Correct of Not
int check_available_seat_sl(int id , vector<Trains> &train);         // Object as an Parameter 
int check_available_seat_ac(int id , vector<Trains> &train);         // Object as an Parameter
void Admin(string id , int pass , vector<Trains> &train);
...
int main()
{
	...
	cout << "\t     ******************************************************\n";
	vector<Trains> train= { Trains(12596,"Humsafar",1000,400,"NDLS","LKO",0,2) , Trains(12531,"Intercity",600,200,"LKO","GKP",2,4) , Trains(12555,"Express",800,300,"NDLS","GKP",0,4)};
	do
	{
        level1:
		cout << "\n\t Choose an Option : \n";
		...
		switch(choice)
		{
			case '1':
				...
				cout << "\t     ******************************************************\n";
				count = 0;
				for(i=0; i<train.size(); i++) // Loop To Display Available Trains
				{
...
int is_available(int train_num, vector<Trains> &train)
{
	for(int i=0; i<train.size(); i++)                // Loop to Iterate According to No. of Trains in The System
		{
...
int check_available_seat_ac(int id , vector<Trains> &train)
{
	if(train[id].ac != 0)
	{
		return 1;
...
int check_available_seat_sl(int id , vector<Trains> &train)
{
	if(train[id].sl != 0)
	{
		return 1;
...
void Admin(string id , int pass , vector<Trains> &train)
{
	...
	{
		cout << "\n\t Wronf ID or Password ";
		return;
	}
	cout << "\n\t     ******************************************************\n";
	cout << "\t                       Admin Panel \n";
	cout << "\t     ******************************************************\n";
	int train_no , ac , sl , source_id , dest_id , ac_fare , sl_fare;
	string train_name , source , dest;
	char choice;
	cout << "\n\t Enter 1 to Add Train to Database : ";
	cout << "\n\t Enter 2 to Update Existing No. of Seats to Database : ";
	cin >> choice;
	switch (choice)
	{
	case '1':
	{
		...
		cout << "\n\t Enter Dest ID :  ";
		cin >> dest_id;
		train.push_back(Trains(train_no ,train_name ,  ac_fare , sl_fare , source , dest, source_id , dest_id));
		break;
	}
	case '2':
	{
		char clas;
		int seats;
		cout << "\n\t Enter Train No. To Update Seats : ";
		cin >> train_no;
		int id = is_available(train_no , train);
		if(id)
		{
			id = id-1;
			cout << "\n\t Enter Class : ";
